// AuthGuard.tsx — משתמש ב-Redux במקום לפענח טוקן ישירות
...
interface AuthGuardProps {
  ...
  userType?: 'customer' | 'representative' | 'admin';
}
...
export const AuthGuard: React.FC<AuthGuardProps> = ({ children, userType }) => {
  ...
  // לא מחובר — הפנה לדף הכניסה המתאים
  if (!isAuthenticated) {
    const loginPath =
      userType === 'representative' ? '/RepresentativeLogin' :
      userType === 'admin'          ? '/admin-login' :
                                      '/login';
    return <Navigate to={loginPath} replace />;
  }

  // מחובר אבל כסוג משתמש שגוי (לקוח מנסה להיכנס לדף נציג וכו')
  if (userType && currentUserType !== userType) {
    console.warn(`Role mismatch. Expected: ${userType}, Got: ${currentUserType}`);
