...
const WaitingRoomStyled: React.FC<WaitingRoomProps> = ({ session, customerName, elapsed, waitTime, onCancel }) => {
  ...
  const numericWait = typeof waitTime === 'number' ? waitTime : 5;
  const totalEstimatedSeconds = (numericWait * 60) + elapsed;
  ...
  return (
    <div className="waiting-page" dir="rtl">
      <div className="wr-card">
        ...
        <h1 className="wr-title">מיד נתחבר, {customerName}</h1>
        ...
        <div className="wr-stats">
          ...
          <div className="wr-stat">
            ...
            <div className="stat-main">
              {typeof waitTime === 'number' ? `~ ${waitTime} דק׳` : waitTime}
            </div>
            <div className="stat-sub">זמן משוער שנותר</div>
          </div>
          <div className="wr-stat">
            <div className="stat-icon">👥</div>
            <div className="stat-main">{session ? 'נמצא בתור' : 'מעבד...'}</div>
            <div className="stat-sub">סטטוס פנייה</div>
          </div>
        </div>
        ...
        <div className="wr-progress-wrap">
          ...
          <p className="wr-progress-note">הנציגים שלנו בודקים את פנייתך. אל תסגור את הדף ✨</p>
        </div>

        <div className="wr-actions">
           <button className="btn-cancel" onClick={onCancel}>ביטול פנייה</button>
        </div>
      </div>
    </div>
  );
