// useChat.hook.ts — קצר, משתמש ב-Slice + SignalR
import { useEffect, useCallback } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { useNavigate } from 'react-router-dom';
import { HubConnectionBuilder, HubConnection } from '@microsoft/signalr';
import { AppDispatch, RootState } from '../store/index';
import { fetchChatHistory, sendMessageThunk, addMessageRealtime } from '../store/slices/Chatmessage.slice';
import { closeSessionThunk } from '../store/slices/Chatsession.slice';
import { SenderType } from '../types/chatMessage.types';
import { useState } from 'react';

export const useChat = (sessionId: number, senderType: SenderType) => {
  const dispatch = useDispatch<AppDispatch>();
  const navigate = useNavigate();
  const [connection, setConnection] = useState<HubConnection | null>(null);

  const { messages, loading } = useSelector((state: RootState) => state.chatMessage);

  // טעינת היסטוריה מה-Store
  useEffect(() => {
    if (!sessionId) return;
  ...
  useEffect(() => {
    ...
    connection.start().then(() => {
      ...
      // הודעה שהגיעה מ-SignalR → נשמרת ב-Store דרך reducer
  ...
  }, [connection, sessionId, dispatch, senderType, navigate]);
  ...
  }, [dispatch, sessionId, senderType]);

  const handleCloseSession = useCallback(async () => {
    if (window.confirm('האם אתה בטוח שברצונך לסגור את השיחה?')) {
      await dispatch(closeSessionThunk(sessionId));
      navigate('/representative-dashboard');
    }
  }, [dispatch, sessionId, navigate]);

  return { messages, loading, sendMessage: handleSendMessage, closeSession: handleCloseSession };
};
